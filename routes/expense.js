...
exports.saveexpense = function(req, res, next){
    ...
    if (!uid || !dt || !typ || !amt) {
        return res.status(422).send({ success: false, message: 'Data via Post esta incompleta o incorrecta.' });
    } else {
...
exports.expensereport = function(req, res, next){
    ...
    if (!uid || !rptype) {
        return res.status(422).send({ error: 'Data via Post esta incompleta o incorrecta.'});
	}else if(rptype === 'opt2' && !fromdt && !todt){
		return res.status(422).send({ error: 'falta la fecha De o Hasta.'});
	}else if(fromdt > todt){   
		 return res.status(422).send({ error: 'fecha De no puede ser mayor a la fecha Hasta.'});
	}else{
